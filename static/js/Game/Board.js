                ...
                switch (this.#board[y][x]) {
                    ...
                    default:
                        if (x != 0 && y != 0 && y != this.#board.length - 1) {
                            ...
                            frame.position.set(
                                x * (this.#itemSize + 2) - this.#board[y].length * (this.#itemSize / 2) - this.#itemSize / 2 - 1,
                                y * (this.#itemSize + 2) - this.#board[y].length * (this.#itemSize / 2),
                                0
                            );
                            this.gameObject.add(frame);
                        } else if (x != 0 && y != 0 && x != this.#board[y].length - 1) {
                            const horizontalFrame = new THREE.Mesh(
                                this.#frameHorizontalGeometry,
                                new THREE.MeshBasicMaterial({
                                    color: this.#boardMaterial,
                                    side: THREE.DoubleSide,
                                    wireframe: false,
                            ...
                            horizontalFrame.position.set(
                                x * (this.#itemSize + 2) - this.#board[y].length * (this.#itemSize / 2),
                                y * (this.#itemSize + 2) - this.#board[y].length * (this.#itemSize / 2) - this.#itemSize / 2 - 1,
                                0
                            );
